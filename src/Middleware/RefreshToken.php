...
namespace HamishVexFleming\KeycloakApi\Middleware;
...
class RefreshToken {
	...
	public function __invoke( callable $handler ) {
		return function ( RequestInterface $request, array $options ) use ( $handler ) {
			...
			return $this->refreshTokenIfNeeded( $token, $options )->then(
				function ( array $cred ) use ( $request, $handler, $options ) {
				...
				function ( ResponseInterface $response ) {
					if ( $response->getStatusCode() >= 400 ) {
						$this->tokenStorage->saveToken( array() );
					}
	...
	public function getAccessToken( $credentials, $refresh, $options ) {
		...
		return $httpClient->requestAsync( 'POST', $url, array( 'form_params' => $params ) )->then(
			function ( ResponseInterface $response ) {
